...
// field это тот же item в цикле выше
   ...
   hasError(result) {
      ...
      return result.error || !response || !response.tokens || !response.tokens.accessToken || !response.tokens.refreshToken || !response.user || !response.user.name || !response.user.lastName || !response.user.id;
   }
